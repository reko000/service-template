...
- **ユーザーストーリー駆動 (User Story Driven)**: 単なる技術タスクをこなすのではなく、「誰が・何の価値を得るか（As a / I want / So that）」を起点に設計すること。
- **実環境構築の義務 (Production Infrastructure First)**: エミュレータやモックで動いただけでタスクを完了と見なしてはならない。本番へデプロイするための設定ファイル（例: `firebase.json`）、バケットやDBの構成、誰でもすぐに試せる `.env.example` などのインフラ準備が完全に含まれているかをPR送付前に必ず確認すること。
