...
2. **ユーザーストーリーの充足**: 開発タスクを機械的にこなすのではなく、Issue冒頭の `As a / I want / So that` を読み込み、「そのインフラ・機能アーキテクチャで本当にビジネス価値を提供できるか」を常に意識して設計する
3. **AI.md の参照**: 対象featureの `AI.md` があれば熟読し、局所ルールを遵守する
4. **垂直分割の徹底**: すべての実装は `features/{feature-name}/` 内で完結させる
5. **横方向インポート禁止**: `features/A` は `features/B` を直接インポートしない
6. **Slotsパターン**: feature間合成には `ReactNode` 型のpropsを使う
7. **Smart/Dumb分離**: データフェッチロジックとUIを分離する
...
4. **インフラ設定の包含**: 仮想環境・ローカルモックで「動いた」として満足せず、実際にデプロイする際に必要となるインフラ設定（`firebase.json`, セキュリティルール、`.env.example` の補完等）を実装に必ず含めること。
